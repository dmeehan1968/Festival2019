  ...
  return (
    <article className={styles.eventcard}>
      <section className={styles.meta}>
        ...
        {opening_times && opening_times.length && <p className={styles.opening_times}>
          {opening_times.map(open => {
            return <span key={open.id}>{moment(open.start).format('ddD')}</span>
          }).reduce((acc, el) => [ acc, ', ', el ])}
        </p> || null}
        {venue && venue.regions && venue.regions.length && <p className={styles.regions}>
          {venue.regions.map(region => {
            return <span key={region.id}>{region.description}</span>
          }).reduce( (acc, el) => [ acc, ', ', el])}
        </p> || null}
        {disciplines && disciplines.length && <p className={styles.disciplines}>
