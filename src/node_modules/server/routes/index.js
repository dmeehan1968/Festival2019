...
export function get(req, res, next) {
  ...
  return fetchData(db)
    .then(getStore)
    .then(renderPage.bind(null, req.url, routerContext))
...
const fetchData = (db) => {
  const requests = {
    ...
    venues: db.models.venues.findAll().then(sequelizeArrayToJSON),
...
const renderPage = (url, context, store) => {
  const { filters, ...initialState } = store.getState()
  return Html(
